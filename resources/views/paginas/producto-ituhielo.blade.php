...
<div aria-label="breadcrumb" class="py-1 font-size-14" style="background-color: #FAFAFA !important;">
    <ol class="breadcrumb container">
        ...
        <li class="breadcrumb-item"><a href="{{ route('ituhielo') }}" class="text-decoration-none">Ituhielo</a></li>
                        ...
                        @if (count($subcategories))
                            @foreach ($subcategories as $sc)
                                <li class="py-2 @if($sc->id == $product->subCategory->id) active @endif"> 
                                    <a href="#" class="toggle d-block p-2 text-decoration-none  text-decoration-none text-blue-dark font-size-14 @if($sc->id == $product->subCategory->id) text-blue @endif">{{$sc->name}}</a>
                                    <ul class="ps-0 {{ ($sc->id == $product->subCategory->id) ? '' : 'rd-none' }}" style="list-style: none">
                                        @foreach ($sc->products as $cp)
                                            <li class="ps-4 py-2">
                                                <a href="{{ route('subcategoria-ituhielo-producto', ['product' => $cp->id]) }}" class="text-blue-dark text-decoration-none font-size-14 d-inline-block fw-bold @if($cp->id == $product->id) text-blue @endif" style="@if($cp->id == $product->id) font-weight: bold; @endif">{{$cp->name}}</a>
                                            </li>
                                        @endforeach
                                    </ul>
                                </li>                        
            ...
            <section class="producto col-sm-12 col-md-9 font-size-14 order-sm-1 order-md-2">
                <div class="row mb-5">
                    ...
                    <div class="col-sm-12 col-md-6 d-flex flex-column justify-content-between">
                        <div class="">
                            <strong class="text-blue font-size-14 d-inline-block mb-2">{{ $product->subCategory->name }}</strong>
                        ...
                        <div class="">
                            <div class="d-flex justify-content-sm-center justify-content-md-start flex-wrap">
                                <a href="{{ route('contacto') }}" class="me-sm-0 me-md-3 mb-sm-3 mb-md-0 px-sm-3 px-md-4 btn ficha-tecnica rounded-pill font-size-13 text-uppercase bg-blue text-white">consultar</a>
                                @if ($product->extra)
                                    <a href="{{ route('ficha-tecnica', ['id'=> $product->id]) }}" class="me-sm-0 me-md-3 mb-sm-3 mb-md-0 px-sm-3 px-md-4 d-flex justify-content-between btn ficha-tecnica rounded-pill font-size-13 align-items-center text-blue" style="border: 2px solid #6582A3;">Ficha Técnica <i class="fas fa-download ms-3"></i></a>  
                                @endif
                            </div>   
                        </div>
                    </div>
                            ...
                            <div class="productos-relacionados row">
                                @foreach ($product->subCategory->products()->where('id', '<>', $product->id)->take(3)->get() as $rp)
                                    <div class="col-sm-12 col-md-4">
                                        @includeIf('paginas.partials.producto-ituhielo', ['p' => $rp])
                                    </div>
                                @endforeach
                            </div>
                        </div>  
