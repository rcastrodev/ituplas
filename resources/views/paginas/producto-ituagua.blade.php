                    ...
                    @isset($products)
                        @if (count($products))
                            @foreach ($products as $p)
                                <li class="ps-4 py-2">
                                    <a href="{{ route('subcategoria-ituagua-producto', ['product' => $p->id]) }}" class="text-blue-dark text-decoration-none font-size-14 d-inline-block fw-bold @if($p->id == $product->id) text-blue @endif" style="@if($p->id == $product->id) font-weight: bold; @endif">{{$p->name}}</a>
                                </li>                     
                            @endforeach
                        @endif
                    @endisset
                            ...
                            <div class="productos-relacionados row">
                                @foreach ($product->subCategory->products()->where('id', '<>', $product->id)->take(3)->get() as $rp)
                                    <div class="col-sm-12 col-md-4">
                                        @includeIf('paginas.partials.producto-ituagua', ['p' => $rp])
                                    </div>
                                @endforeach
                            </div>
                        </div>  
