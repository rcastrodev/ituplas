...
<div aria-label="breadcrumb" class="py-1 font-size-14" style="background-color: #FAFAFA !important;">
	<div class="container">
		<ol class="breadcrumb">
            ...
            <li class="breadcrumb-item"><a href="{{ route('ituagua') }}" class="text-decoration-none font-size-13">Ituagua</a></li>
                            ...
                            <li class="py-2 @if($c->id == $subCategory->id) active @endif" > 
                                ...
                                <ul class="p-0 {{ ($c->id == $subCategory->id) ? '' : 'rd-none' }}" style="list-style: none">
                                    @foreach ($c->products as $cp)
                                        <li class="py-2">
                                            <a href="{{ route('subcategoria-ituagua-producto', ['product' => $cp->id]) }}" class="text-blue-dark text-decoration-none ps-4 font-size-14 d-inline-block fw-bold">{{$cp->name}}</a>
                                        </li>
                                    @endforeach
                                </ul>
                            </li>                        
                            ...
                            @if (count($products))
                                @foreach ($products as $p)
                                    <div class="col-sm-12 col-md-4 mb-3">
                                        @include('paginas.partials.producto-ituagua', ['p' => $p])
                                    </div>
                                @endforeach    
                            @else
